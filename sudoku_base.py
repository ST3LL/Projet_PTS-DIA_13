...
class Sudoku:
    ...
    def grid_to_string(self) -> str:
        moveset = set(string.printable[:self.dim + 1])
        d_map = {i: move for i, move in enumerate(sorted(moveset))}
        return ''.join(''.join([d_map[x] for x in row]) for row in self.grid)

    def string_to_tensor(self, string_grid: str) -> np.array:
        moveset = set(string.printable[:self.dim + 1])
        d_map = {move: i for i, move in enumerate(sorted(moveset))}
        flat = np.array(
            [np.reshape([d_map[move] for move in string_grid], (self.dim, self.dim))]
        )
